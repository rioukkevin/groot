  ...
  await payload.updateGlobal({
    ...
    data: {
      ...
      strings: [
        ...
        { key: "ask.matches", text: "matches" },
        { key: "ask.unsure.short", text: "unsure" },
        { key: "ask.tierLight", text: "light model" },
        { key: "ask.tierFull", text: "neural net" },
        { key: "ask.tierLightSized", text: "light model · 200 KB" },
        { key: "ask.tierFullSized", text: "neural net · {size}" },
        { key: "ans.availability", text: "Short answer: ⟦{headline}⟧ Here is what changed this month:" },
        { key: "ans.rates", text: "⟦{label} · {value}⟧ — the other rates, for the record:" },
        { key: "ans.contact", text: "Email is the fastest: ⟦{email}⟧ — or answer the four questions below and it lands in my inbox." },
        { key: "ans.channel", text: "⟦{label} · {value}⟧ — or use the form below." },
        { key: "ans.profile", text: "⟦{name}⟧ — {tagline}, based in ⟦{location}⟧." },
        { key: "ans.roles", text: "⟦{count} roles⟧ since {first}, the latest at ⟦{where}⟧ ({when}) — ↑↓ opens one:" },
        { key: "ans.role", text: "{when}, {where}: ⟦{what}⟧." },
        { key: "ans.projects", text: "⟦{count} side projects⟧ — {names} and more below. ↑↓ opens a case study:" },
        { key: "ans.project", text: "⟦{name}⟧ — {what}" },
        { key: "ans.skills", text: "⟦{count} technologies⟧ across {groups} groups — the ones I reach for without thinking:" },
        { key: "ans.skillYes", text: "Yes — ⟦{item}⟧ is part of my {group} stack:" },
        { key: "ans.skillNear", text: "Not in the core stack, but ⟦{tech}⟧ shows up in {where}. The stack itself:" },
        { key: "ans.skillNo", text: "⟦{tech}⟧ isn't in my stack. Here is what I do reach for:" },
        { key: "ans.softSkills", text: "The part that isn't typing — ⟦{count} soft skills⟧ in {groups} groups:" },
        { key: "ans.education", text: "⟦{what}⟧ — {where}, {when}. The full path:" },
        { key: "ans.photos", text: "Every block is one terminal cursor — hover to resolve the photo:" },
        { key: "ans.resume", text: "A plain-text CV, one file:" },
        { key: "ans.theme", text: "Pick a palette — ←→ ↑↓ then ↵:" },
        { key: "ans.themeSet", text: "Theme: ⟦{theme}⟧." },
        { key: "ans.voice", text: "Pick a register — ↑↓ then ↵:" },
        { key: "ans.voiceSet", text: "Voice: ⟦{voice}⟧." },
        { key: "ans.help", text: "Everything I can pull up:" },
        { key: "ans.more", text: "One level deeper — ⟦{command}⟧:" },
        { key: "ans.searchHit", text: "Closest match in the site's content: ⟦{name}⟧." },
        { key: "ans.searchMany", text: "{count} projects mention that — the lit rows. ↑↓ opens one:" },
        { key: "reply.greeting", text: "Hey! Ask me anything about my work — /help lists the shortcuts." },
        { key: "reply.thanks", text: "Anytime. /contact if you want to take it further." },
        { key: "reply.goodbye", text: "See you. The email is in /contact if something comes up." },
        { key: "reply.howAreYou", text: "Good, thanks — busy shipping. /now says what changed this month." },
        { key: "reply.compliment", text: "Thanks — this is the ninth version; the earlier ones are under /projects." },
        { key: "reply.affirm", text: "Go on — ask away, or pick a command from /help." },
        { key: "reply.deny", text: "Alright. /help lists what else I can pull up." },
        { key: "reply.aboutSite", text: "This is my portfolio, built as a shell: Next.js in front, a CMS behind, and a neural net on your device that routes each question to the right command — nothing you type leaves the page. Reading you right now: the ⟦{tier}⟧." },
        { key: "reply.moreWhat", text: "More about what? Name it — a project, a role, the rates — or pick from /help." },
        { key: "reply.pickAbove", text: "Pick one from the list above — ↑↓ then ↵ — or ask about it by name." },
        { key: "reply.projectMore", text: "←→ flips the screenshots and ↑↓ scrolls the write-up above. The other projects are under /projects." },
        { key: "reply.language", text: "Both — shift+tab switches the whole shell to {other}." },
        { key: "reply.switching", text: "Switching to {other} — shift+tab brings {current} back." },
        { key: "nn.s0", text: "waking the neurons…" },
        { key: "nn.s1", text: "growing {buckets} synapses…" },
        { key: "nn.s2", text: "learning {labels} intents…" },
        { key: "nn.s3", text: "wiring both languages…" },
        { key: "nn.s4", text: "settling the weights…" },
        { key: "nn.many", text: "400 000+" },
        { key: "nn.done", text: "neural net online · {size}" },
        { key: "nn.failed", text: "staying on the light model" },
        { key: "nn.skipped", text: "data saver on · light model" },
        { key: "ans.projectLatest", text: "The most recent: ⟦{name}⟧ ({year}) — {what}" },
        { key: "ans.projectFirst", text: "The first one: ⟦{name}⟧ ({year}) — {what}" },
        { key: "ans.roleLatest", text: "The current one —" },
        { key: "ans.roleFirst", text: "The first one —" },
        { key: "ans.probably", text: "Not sure I read that right — the closest I have:" },
        { key: "hint.past", text: "earlier answer · read-only" }
      ],
      introHints: [
  ...
  await payload.updateGlobal({
    ...
    data: {
      ...
      strings: [
        ...
        { key: "ask.matches", text: "correspondances" },
        { key: "ask.unsure.short", text: "incertain" },
        { key: "ask.tierLight", text: "modèle léger" },
        { key: "ask.tierFull", text: "réseau de neurones" },
        { key: "ask.tierLightSized", text: "modèle léger · 200 Ko" },
        { key: "ask.tierFullSized", text: "réseau de neurones · {size}" },
        { key: "ans.availability", text: "Réponse courte : ⟦{headline}⟧ Voici ce qui a changé ce mois-ci :" },
        { key: "ans.rates", text: "⟦{label} · {value}⟧ — les autres tarifs, pour mémoire :" },
        { key: "ans.contact", text: "L'email est le plus rapide : ⟦{email}⟧ — ou répondez aux quatre questions ci-dessous et ça arrive dans ma boîte." },
        { key: "ans.channel", text: "⟦{label} · {value}⟧ — ou passez par le formulaire ci-dessous." },
        { key: "ans.profile", text: "⟦{name}⟧ — {tagline}, basé à ⟦{location}⟧." },
        { key: "ans.roles", text: "⟦{count} postes⟧ depuis {first}, le dernier chez ⟦{where}⟧ ({when}) — ↑↓ pour en ouvrir un :" },
        { key: "ans.role", text: "{when}, {where} : ⟦{what}⟧." },
        { key: "ans.projects", text: "⟦{count} projets perso⟧ — {names} et d'autres ci-dessous. ↑↓ ouvre une étude de cas :" },
        { key: "ans.project", text: "⟦{name}⟧ — {what}" },
        { key: "ans.skills", text: "⟦{count} technologies⟧ en {groups} groupes — celles que je prends sans réfléchir :" },
        { key: "ans.skillYes", text: "Oui — ⟦{item}⟧ fait partie de ma stack {group} :" },
        { key: "ans.skillNear", text: "Pas dans la stack de base, mais ⟦{tech}⟧ apparaît dans {where}. La stack elle-même :" },
        { key: "ans.skillNo", text: "⟦{tech}⟧ n'est pas dans ma stack. Voici ce que j'utilise vraiment :" },
        { key: "ans.softSkills", text: "La partie du métier qui n'est pas du clavier — ⟦{count} soft skills⟧ en {groups} groupes :" },
        { key: "ans.education", text: "⟦{what}⟧ — {where}, {when}. Le parcours complet :" },
        { key: "ans.photos", text: "Chaque bloc est un curseur de terminal — survolez pour révéler la photo :" },
        { key: "ans.resume", text: "Un CV en texte brut, un seul fichier :" },
        { key: "ans.theme", text: "Choisissez une palette — ←→ ↑↓ puis ↵ :" },
        { key: "ans.themeSet", text: "Thème : ⟦{theme}⟧." },
        { key: "ans.voice", text: "Choisissez un registre — ↑↓ puis ↵ :" },
        { key: "ans.voiceSet", text: "Voix : ⟦{voice}⟧." },
        { key: "ans.help", text: "Tout ce que je peux afficher :" },
        { key: "ans.more", text: "Un niveau plus loin — ⟦{command}⟧ :" },
        { key: "ans.searchHit", text: "Le plus proche dans le contenu du site : ⟦{name}⟧." },
        { key: "ans.searchMany", text: "{count} projets en parlent — les lignes allumées. ↑↓ pour en ouvrir un :" },
        { key: "reply.greeting", text: "Salut ! Posez-moi une question sur mon travail — /help liste les raccourcis." },
        { key: "reply.thanks", text: "Avec plaisir. /contact si vous voulez aller plus loin." },
        { key: "reply.goodbye", text: "À bientôt. L'email est dans /contact si besoin." },
        { key: "reply.howAreYou", text: "Bien, merci — occupé à livrer. /now dit ce qui a changé ce mois-ci." },
        { key: "reply.compliment", text: "Merci — c'est la neuvième version ; les précédentes sont dans /projects." },
        { key: "reply.affirm", text: "Allez-y — posez votre question, ou choisissez une commande dans /help." },
        { key: "reply.deny", text: "Très bien. /help liste ce que je peux afficher d'autre." },
        { key: "reply.aboutSite", text: "C'est mon portfolio, construit comme un shell : Next.js devant, un CMS derrière, et un réseau de neurones sur votre appareil qui envoie chaque question vers la bonne commande — rien de ce que vous tapez ne quitte la page. Celui qui vous lit en ce moment : le ⟦{tier}⟧." },
        { key: "reply.moreWhat", text: "Plus sur quoi ? Nommez-le — un projet, un poste, les tarifs — ou choisissez dans /help." },
        { key: "reply.pickAbove", text: "Choisissez dans la liste au-dessus — ↑↓ puis ↵ — ou demandez-le par son nom." },
        { key: "reply.projectMore", text: "←→ fait défiler les captures et ↑↓ le texte au-dessus. Les autres projets sont dans /projects." },
        { key: "reply.language", text: "Les deux — shift+tab bascule tout le shell en {other}." },
        { key: "reply.switching", text: "On passe en {other} — shift+tab ramène le {current}." },
        { key: "nn.s0", text: "réveil des neurones…" },
        { key: "nn.s1", text: "{buckets} synapses en croissance…" },
        { key: "nn.s2", text: "apprentissage de {labels} intentions…" },
        { key: "nn.s3", text: "câblage des deux langues…" },
        { key: "nn.s4", text: "les poids se stabilisent…" },
        { key: "nn.many", text: "400 000+" },
        { key: "nn.done", text: "réseau de neurones en ligne · {size}" },
        { key: "nn.failed", text: "on reste sur le modèle léger" },
        { key: "nn.skipped", text: "économie de données · modèle léger" },
        { key: "ans.projectLatest", text: "Le plus récent : ⟦{name}⟧ ({year}) — {what}" },
        { key: "ans.projectFirst", text: "Le premier : ⟦{name}⟧ ({year}) — {what}" },
        { key: "ans.roleLatest", text: "Le poste actuel —" },
        { key: "ans.roleFirst", text: "Le premier —" },
        { key: "ans.probably", text: "Pas sûr d'avoir bien lu — le plus proche que j'ai :" },
        { key: "hint.past", text: "réponse précédente · lecture seule" }
      ],
      introHints: [
